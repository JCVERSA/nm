...
export default function SidebarToggleIcon({ isOpen, strokeWidth = 1.5, className }: SidebarToggleIconProps) {
  return (
    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" className={className}>
      <path d={OUTER} fill="currentColor" stroke="currentColor" strokeWidth={strokeWidth} strokeLinecap="round" strokeLinejoin="round" />
      <motion.path d={isOpen ? PANEL_OPEN : PANEL_CLOSED} animate={{ d: isOpen ? PANEL_OPEN : PANEL_CLOSED }} style={{ fill: "var(--background, #0f0f10)" }} transition={{ duration: 0.35, ease: [0.4, 0, 0.2, 1] }} />
    </svg>
  );
}
